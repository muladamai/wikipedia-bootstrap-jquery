...
$(".dropdown-menu li a").click(function(){
  ...
  console.log(hrefText);
	...
	success: function (data, textStatus, jqXHR) {
		...
		$.ajax({
			type: "GET",	
			url: "http://"+(lang=="en"?"id":"id")+".wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page="+locationwiki+"&callback=?",
		...
		$.ajax({
			type: "GET",	
			url: "https://"+(lang=="en"?"id":"id")+".wikipedia.org/w/api.php?action=query&prop=extracts&titles="+locationwiki+"&format=json&prop=pageimages&pithumbsize=220&callback=?",
			//url: "http://id.wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page="+locationwiki+"&callback=?",
			//https://id.wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page=semarang&callback=?
			contentType: "application/json; charset=utf-8",
			async: true,
