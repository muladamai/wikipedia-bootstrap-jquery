...
$(".dropdown-menu li a").click(function(){
  ...
  console.log(hrefText);
	...
	success: function (data, textStatus, jqXHR) {
		...
		if (lang=="en") locationwiki=data.query.pages[i].langlinks[0]['*'];
		else locationwiki=data.query.pages[i].title;
		console.log(locationwiki);
		//return;
		$.ajax({
			type: "GET",	
			url: "http://"+(lang=="id"?"en":"en")+".wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page="+locationwiki+"&callback=?",
		...
		$.ajax({
			type: "GET",	
			url: "https://"+(lang=="id"?"en":"en")+".wikipedia.org/w/api.php?action=query&prop=extracts&titles="+locationwiki+"&format=json&prop=pageimages&pithumbsize=220&callback=?",
			//url: "http://id.wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page="+locationwiki+"&callback=?",
			//https://id.wikipedia.org/w/api.php?action=parse&format=json&prop=text&section=0&page=semarang&callback=?
			contentType: "application/json; charset=utf-8",
			async: true,
